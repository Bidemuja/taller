/**
Clase encargada de gestionar las cargas familiares de un trabajador.
/
class CargasFamiliaresManager {
*/
...
/** @urlParametros --> Obtiene los parámetros de la URL. 
*/

/** Lista de Cargas familiares
 * Obtiene la lista de cargas familiares.
*/
function listarCargasFamiliares() {
    ...
    fetch("http://localhost:3000/api/carga_familiar?_size=100", requestOptions).then(response => response.json()).then((json) => {
...
/**
Completa una fila de la tabla de cargas familiares.
*/
function completarFila(element, index, arr) {
    if (element.rut_trabajador === id_contacto_url) {
        arr[index] = document.querySelector('#tbl_cargas_familiares tbody').innerHTML += `<tr>
          <td>${
            element.rut_carga
        }</td>
          <td>${
            element.nombre_carga
        }</td>
          <td>${
            element.sexo_carga
        }</td>
          <td>${
            element.parentesco_carga
        }</td>
          <td>
    <a href='eliminar-carga-familiar.html?id=${
            element.rut_carga
        }&nombre=${
            element.nombre_carga
        }&idtrab=${
    ...
    <a href='actualizar-carga-familiar.html?id=${
            element.rut_carga
        }&idtrab=${
            element.rut_trabajador
...
function crearCargaFamiliar() {
    ...
    // Variables con los datos de formulario para crear carga familiar
    ...
    fetch("http://localhost:3000/api/carga_familiar", requestOptions).then(response => {
        if (response.ok) {
...
/**
Consulta los datos de una carga familiar.
*/
function consultarDatosCargaFamiliar(rut_carga) {
    ...
    fetch("http://localhost:3000/api/carga_familiar/" + rut_carga, requestOptions).then(response => response.json()).then((json) => json.forEach(completarFormulario)).catch(error => console.log('error', error));
}
// Completar formulario
/**
Completa el formulario con los datos de una carga familiar.

@param {object} element - El elemento de carga familiar.
*/
...
// Obtener los datos de la carga familiar a actualizar
/**
Obtiene el ID de la carga familiar a actualizar y completa los datos.
*/
...
// Actualizar los datos de la carga familiar con el método patch
function actualizarCargaFamiliar() {
    ...
    fetch("http://localhost:3000/api/carga_familiar/" + txt_rut_carga, requestOptions).then(response => {
        if (response.ok) {
...
// Obtiene el id de la carga familiar a eliminar
...
/**Elimina la carga familiar de acuerdo a la ID obtenida desde la URL */
function eliminarCargaFamiliar() {
    ...
    fetch("http://localhost:3000/api/carga_familiar/" + rut_carga_eliminar, requestOptions).then(response => {
        if (response.ok) {
...
/**
Envía la URL de las cargas familiares.
*/
...
/**Mantiene en la URL el valor de id del dueño de la sesión en curso */
...
/**
 * Valida los campos del formulario antes de crear una nueva carga.
 */
